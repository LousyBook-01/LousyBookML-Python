"""Test cases for Linear Regression module."""

import numpy as np
import pytest
from LousyBookML.linear_regression import LinearRegression
from LousyBookML.linear_regression.losses import (
    mean_squared_error,
    mean_absolute_error,
    r2_score
)
from LousyBookML.linear_regression.utils import (
    standardize_data,
...
def test_r2_score():
    ...
    assert r2_score(y_true, y_mean) == pytest.approx(0.0, abs=1e-6)
...
def test_standardize_data():
    ...
    X_const_std, _, _ = standardize_data(X_const)
...
def test_add_polynomial_features():
    ...
    # Test degree 3
    X_poly = add_polynomial_features(X, degree=3)
